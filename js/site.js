...
(function () {
  ...
  function apply(d) {
    ...
    try {
      localStorage.setItem('rr_theme', JSON.stringify({
        accent: d.theme.accent || '',
        heroImage: (d.hero && d.hero.image) || ''
      }));
    } catch (e) {}
    var grain = $('grain');
    ...
    var heroImg = $('heroImage'), heroWrap = $('heroImageWrap'), heroGrid = $('heroGrid');
    if (heroImg && d.hero.image) {
      if (heroImg.getAttribute('src') !== d.hero.image) heroImg.setAttribute('src', d.hero.image);
    } else if (heroWrap && heroGrid) {
