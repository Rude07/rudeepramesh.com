...
(function () {
  ...
  function renderBrands(d) {
    ...
    // Row A: original order. Row B: rotated halfway so rows don't mirror.
    var half = Math.ceil(list.length / 2);
    var rotated = list.slice(half).concat(list.slice(0, half));
    var oneA = list.map(tile).join('');
    var oneB = rotated.map(tile).join('');
    trackA.innerHTML = oneA + oneA;   // duplicate so the -50% keyframe loops seamlessly
    trackB.innerHTML = oneB + oneB;
  }
