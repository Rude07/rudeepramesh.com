...
(function () {
  ...
  function renderBrands(d) {
    ...
    // Fill each track with enough repeats of one "set" that a single set is wider
    // than the viewport, then animate exactly -50% (i.e. one full copy of the
    // doubled content) so the loop is perfectly seamless with no jump.
    function fill(track, oneSet, durVar, baseSpeedPxPerSec) {
      track.innerHTML = oneSet;                 // measure one set
      var vw = window.innerWidth || 1200;
      // repeat the set until it comfortably exceeds the viewport width
      var guard = 0;
      while (track.scrollWidth < vw * 1.2 && guard < 12) {
        track.innerHTML += oneSet; guard++;
      }
      var oneWidth = track.scrollWidth;          // width of the repeated "first copy"
      track.innerHTML = track.innerHTML + track.innerHTML;  // duplicate for -50% loop
      // duration scales with content width so speed feels constant regardless of count
      var dur = Math.max(18, Math.round(oneWidth / baseSpeedPxPerSec));
      track.style.setProperty(durVar, dur + 's');
    }
    fill(trackA, setA, '--durA', 70);   // ~70px/sec
    fill(trackB, setB, '--durB', 48);   // slower bottom row
  }
