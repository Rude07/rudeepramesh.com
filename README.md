...
3. Get credited by name on the EBS site + 1–2 design-community features —
   these external mentions are what eventually trigger the Knowledge Panel.

---

## Case Study Project Pages (the /work/ section)

Each project is its own page at `rudeepramesh.com/work/<name>/` with its own
title, description, social preview and Google schema — real pages that can
rank on their own.

### How to add / edit a project
1. Go to `/admin/`, log in.
2. In the left sidebar you now have **two** collections: **Case Study
   Projects** and **Website Content**.
3. Open **Case Study Projects** → **New Project** (or click an existing one).
4. Fill in title, category, thumbnail, video link, and the write-up fields
   (overview, challenge, approach, outcome). Set a display **order** (lower
   shows first).
5. **Publish.** The page builds automatically on deploy — live at
   `/work/<title>/` within ~1–2 minutes, added to the sitemap, and listed
   on the `/work/` page.

### The build step (one-time — already configured)
`netlify.toml` now runs `python3 build.py` on every deploy. That script reads
`content/projects/*.json` and generates the `/work/` pages + `sitemap.xml`.
Python is built into Netlify, so nothing to install.

- If a deploy build ever fails, Netlify keeps your **last working version
  live** — the site never goes down.
- Editing projects **locally**: after `npx decap-server`, run `python3
  build.py` to regenerate pages before previewing.
- The homepage nav now has a **STUDIES** link pointing to `/work/`.
